...
function callGemini(prompt_1, apiKey_1) {
    return __awaiter(this, arguments, void 0, function* (prompt, apiKey, model = "gemini-1.5-flash", deviceType = "mobile") {
...
- El frame principal DEBE usar exactamente estas dimensiones: ${config.width}x${config.height}
...
✅ SIEMPRE usa las dimensiones exactas del dispositivo: ${config.width}x${config.height}
                    ...
                    parts: [
                        {
                            text: (() => {
                                ...
                                const isLoginPrompt = prompt.toLowerCase().includes("login") ||
                                    prompt.toLowerCase().includes("sesión") ||
                                    prompt.toLowerCase().includes("password") ||
                                    prompt.toLowerCase().includes("contraseña");
                                const loginExample = isLoginPrompt && deviceType === "mobile"
                                    ? loginSpecificPrompt
                                    : "";
                                console.log(`Prompt tipo: ${isLoginPrompt ? "Login específico" : "General"}, Dispositivo: ${deviceType}`);
                                return (systemPrompt +
                                    devicePrompt +
                                    loginExample +
                                    "\n\nPrompt del usuario: " +
            ...
            try {
                ...
                try {
                    ...
                    if (openBraces !== closeBraces || openBrackets !== closeBrackets) {
                        throw new Error(`JSON truncado - llaves: ${openBraces}/${closeBraces}, corchetes: ${openBrackets}/${closeBrackets}. Aumenta el límite de tokens.`);
                    }
                    ...
                    const preview = responseText.length > 500
                        ...
                        : responseText;
                    throw new Error(`Error al parsear respuesta de Gemini (${parseError instanceof Error ? parseError.message : "parsing error"}): ${preview}`);
                }
            }
            catch (error) {
                lastError = error;
...
function sendUIMessage(type, message, alertType = "error") {
    ...
    if (msg.type === "getContext") {
        ...
        if (selection.length === 1 && selection[0].type === "FRAME") {
            ...
            children.forEach((child) => {
                if ("fills" in child && child.fills && Array.isArray(child.fills)) {
                    child.fills.forEach((fill) => {
                        if (fill.type === "SOLID" && "color" in fill) {
                            ...
                            if (primaryColors.indexOf(colorStr) === -1 &&
                ...
                if (child.name.toLowerCase().indexOf("image") !== -1 ||
            ...
            if (hasButtons)
                description += ", incluye botones";
            if (hasInputs)
                description += ", incluye campos de entrada";
            if (hasImages)
                description += ", incluye imágenes";
            selectedFrame = {
                name: frame.name,
                width: frame.width,
                height: frame.height,
                ...
                styleInfo: {
                    backgroundColor: frame.fills &&
                        ...
                        ? `rgb(${Math.round(frame.fills[0].color.r * 255)}, ${Math.round(frame.fills[0].color.g * 255)}, ${Math.round(frame.fills[0].color.b * 255)})`
                        : "white",
        ...
        try {
            ...
            sendUIMessage("success", `Generando diseño con ${modelDisplay}...`, "success");
...
NOTA: El usuario ha definido reglas personalizadas que ya están incluidas en el prompt anterior. Asegúrate de respetarlas.`;
            ...
            const data = yield callGemini(contextualPrompt, msg.apiKey, msg.model, msg.deviceType);
            ...
            if (msg.selectedFrame) {
                yield updateExistingFrame(data.frames, msg.selectedFrame.id);
            }
            else {
...
function updateExistingFrame(frames, frameId) {
    return __awaiter(this, void 0, void 0, function* () {
        const existingFrame = figma.getNodeById(frameId);
        ...
        if (frames.length > 0) {
            ...
            for (const n of frameDef.nodes) {
                yield addNodeToFrame(existingFrame, n);
            }
        }
    });
}
        ...
        for (const frameDef of frames) {
            ...
            if (!frame) {
                ...
                if (mainFrameNode && mainFrameNode.width && mainFrameNode.height) {
                    console.log("Usando dimensiones del nodo FRAME:", mainFrameNode.width, "x", mainFrameNode.height);
                    ...
                    const hasLargeElements = frameDef.nodes.some((n) => (n.width && n.width > 600) || (n.height && n.height > 600));
                    if (hasLargeElements) {
                ...
                console.log("Frame creado con dimensiones finales:", frame.width, "x", frame.height);
            }
            ...
            if (hasMainFrame && mainFrameIndex !== -1) {
                ...
                yield addNodeToFrame(frame, mainFrameNode);
                // Procesar todos los demás nodos como hijos del frame principal
                console.log(`Procesando ${frameDef.nodes.length - 1} elementos hermanos como hijos del frame principal`);
                for (let i = 0; i < frameDef.nodes.length; i++) {
                    if (i !== mainFrameIndex) {
                        // Omitir el frame principal ya procesado
                        yield addNodeToFrame(frame, frameDef.nodes[i]);
                    }
                }
            }
            else {
            ...
            console.log("Frame procesado con", frame.children.length, "elementos hijos");
        }
    });
}
// Función auxiliar para agregar nodos a un frame
function addNodeToFrame(frame, n) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (n.type === "FRAME" && frame.children.length === 0) {
            console.log("Aplicando propiedades del nodo FRAME principal al frame contenedor");
            ...
            if (n.layoutMode && n.layoutMode !== "NONE") {
                try {
                    ...
                    if (n.primaryAxisAlignItems &&
                        ["MIN", "CENTER", "MAX", "SPACE_BETWEEN"].indexOf(n.primaryAxisAlignItems) !== -1) {
                    ...
                    console.warn("Error configurando auto-layout en frame principal:", layoutError);
                }
            }
            // Aplicar radio de esquinas
            if (n.cornerRadius)
                frame.cornerRadius = n.cornerRadius;
            console.log("Propiedades del FRAME principal aplicadas, ahora procesando sus children");
            // Procesar los children del FRAME principal (puede ser 'children' o 'nodes')
            const nodeChildren = n.children || n.nodes;
            if (nodeChildren && nodeChildren.length > 0) {
                console.log("Procesando", nodeChildren.length, "children del FRAME principal");
                for (const child of nodeChildren) {
                    yield addNodeToFrame(frame, child);
                }
            }
            else {
        ...
        switch (n.type) {
            case "TEXT":
                ...
                const textContent = n.characters || n.text || "Texto";
                ...
                if (n.fontWeight) {
                    ...
                    try {
                        ...
                        console.warn(`No se pudo cargar fuente Inter ${weight}, usando Regular`);
                        yield figma.loadFontAsync({ family: "Inter", style: "Regular" });
                        t.fontName = { family: "Inter", style: "Regular" };
                    }
                }
                ...
                if (n.textAlign) {
                    try {
                        ...
                        if (["LEFT", "CENTER", "RIGHT"].indexOf(align) !== -1) {
                            ...
                            if (lowerAlign === "center")
                                t.textAlignHorizontal = "CENTER";
                            else if (lowerAlign === "right")
            ...
            case "RECTANGLE":
                ...
                const rectWidth = typeof n.width === "string" ? 100 : (_a = n.width) !== null && _a !== void 0 ? _a : 100;
                const rectHeight = typeof n.height === "string" ? 100 : (_b = n.height) !== null && _b !== void 0 ? _b : 100;
            ...
            case "FRAME":
                ...
                f.resize((_c = n.width) !== null && _c !== void 0 ? _c : 100, (_d = n.height) !== null && _d !== void 0 ? _d : 100);
                ...
                // Aplicar radio de esquinas
                if (n.cornerRadius)
                    f.cornerRadius = n.cornerRadius;
                // Procesar children recursivamente si los hay (puede ser 'children' o 'nodes')
                const frameChildren = n.children || n.nodes;
                if (frameChildren && frameChildren.length > 0) {
                    console.log(`Procesando ${frameChildren.length} children del FRAME ${n.type}`);
                    for (const child of frameChildren) {
                        yield addNodeToFrame(f, child);
                    }
                }
                node = f;
                break;
            case "IMAGE":
                ...
                img.resize((_e = n.width) !== null && _e !== void 0 ? _e : 100, (_f = n.height) !== null && _f !== void 0 ? _f : 100);
            ...
            default:
                ...
                df.resize((_g = n.width) !== null && _g !== void 0 ? _g : 100, (_h = n.height) !== null && _h !== void 0 ? _h : 100);
                node = df;
        }
        ...
        if (frame.layoutMode === "NONE") {
            node.x = n.x;
            node.y = n.y;
            console.log(`Posicionando manualmente elemento ${n.type} en (${n.x}, ${n.y})`);
        }
        else {
        ...
        if (n.fills && n.fills.length > 0 && "fills" in node) {
            try {
                console.log(`Aplicando ${n.fills.length} fills a elemento ${n.type}:`, n.fills);
                const figmaFills = n.fills.map((fill) => {
                ...
                if (((_j = firstFill === null || firstFill === void 0 ? void 0 : firstFill.color) === null || _j === void 0 ? void 0 : _j.a) !== undefined && firstFill.color.a < 1) {
            ...
            console.log(`No fills especificados para ${n.type}, aplicando color por defecto`);
            node.fills = [
                {
                    type: "SOLID",
                    color: n.type === "TEXT" ? { r: 0.1, g: 0.1, b: 0.1 } : { r: 1, g: 1, b: 1 },
                },
            ];
        }
        ...
        const debugText = n.characters || n.text || "N/A";
        console.log(`✅ Elemento ${n.type} agregado al frame. Texto: "${debugText}", Dimensiones: ${node.width}x${node.height}`);
        // Si es una imagen, agregar emoji indicativo centrado
        if (n.type === "IMAGE") {
            try {
                const imgIcon = figma.createText();
                // Elegir emoji según el tipo de imagen
                const imageType = ((_k = n.imageUrl) === null || _k === void 0 ? void 0 : _k.toLowerCase()) || "photo";
                const emoji = imageType.includes("avatar") || imageType.includes("profile")
                    ? "👤"
                ...
                imgIcon.fontSize = Math.min((_l = n.width) !== null && _l !== void 0 ? _l : 100, (_m = n.height) !== null && _m !== void 0 ? _m : 100) * 0.3;
                imgIcon.textAlignHorizontal = "CENTER";
                // Centrar perfectamente el emoji
                const iconWidth = imgIcon.fontSize;
                imgIcon.x = n.x + ((_o = n.width) !== null && _o !== void 0 ? _o : 100) / 2 - iconWidth / 2;
                imgIcon.y = n.y + ((_p = n.height) !== null && _p !== void 0 ? _p : 100) / 2 - iconWidth / 2;
